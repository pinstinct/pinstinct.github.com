...
`setweight` 함수는 `tsvector` 항목에 주어진 **가중치**를 부여하여 레이블을 지정하는 데 사용한다. 가중치는 A, B, C, D 중 하나입니다. 이 함수는 일반적으로 제목과 본문처럼 문서의 여러 부분에서 온 항목을 표시하는데 사용한다. 나중에 이 정보는 검색 결과 순위를 매기는데 사용할 수 있다.
