...
In [7]: def outter():
   ...
   ...:     def inner():
   				# nonlocal 키워드를 사용하므로,
   				# outter에 새로 정의된 변수 지역변수 level 사용
   ...:         nonlocal level
   ...:         level += 3
   ...:         print(level)
   			# inner 함수의 호출 결과가 아닌
   			# 함수 자체를 반환
   			# 이때, outter안쪽의 모든환경을 포함해 반환
   			# 함수 자체도 클로져를 갖는다!
   ...:     return inner
...
# outter 함수는 inner 함수를 반환하여 func 전역변수에 할당
# func 전역변수에는 inner 함수자체를 할당받으므로 실행할 수 있는 함수객체
In [8]: func = outter()
...
1. 매개변수로 문자열을 받고, 해당 문자열이 `red`면 `apple`을, `yellow`면 `banana`를, `green`이면 `melon`을, 어떤 경우도 아닐 경우 `I don't know`를 리턴하는 함수를 정의하고, 사용하여 `result`변수에 결과를 할당하고 `print`해본다. [코드 링크]
2. 1번에서 작성한 함수에 `docstring`을 작성하여 함수에 대한 설명을 달아보고, `help(함수명)`으로 해당 설명을 출력해본다. [코드 링크]
3. 한 개 또는 두 개의 숫자 인자를 전달받아, 하나가 오면 제곱, 두개를 받으면 두 수의 곱을 반환해주는 함수를 정의하고 사용해본다. [코드 링크]
4. 두 개의 숫자를 인자로 받아 합과 차를 튜플을 이용해 동시에 반환하는 함수를 정의하고 사용해본다. [코드 링크]
5. 위치인자 묶음을 매개변수로 가지며, 위치인자가 몇 개 전달되었는지를 print하고 개수를 리턴해주는 함수를 정의하고 사용해본다. [코드 링크]
6. 람다함수와 리스트 컴프리헨션을 사용해 한 줄로 구구단의 결과를 갖는 리스트를 생성해본다. [코드 링크]
