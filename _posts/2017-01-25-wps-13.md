...
```shell
# 리터럴 (literal) : 변하지 않는 고정된 데이터 장체의 표현
# 정수형 데이터
In [1]: 7
Out[1]: 7
# 문자열 데이터
...
# 문자열을 입력해주세요 :라는 안내문구를 띄워주도록 input함수를 사용해본다.
# 결과는 var에 할당한다.
...
<ipython-input-4-d0b20e23e9ef> in <module>()
----> 1 lux[50]
...
<ipython-input-5-62091b83abda> in <module>()
----> 1 lux[0] = '손'
...
```shell
In [1]: long_string = 'Introduction and overview of IPython\'s features.'
# [start:] start 부터 마지막까지
In [2]: long_string[10:]
Out[2]: "on and overview of IPython's features."
# [:end] 처음부터 end 까지
In [3]: long_string[:5]
Out[3]: 'Intro'
...
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-3-ca3dd1f46975> in <module>()
----> 1 '%d % %d : %d' % (12, 4, 0)
...
---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
<ipython-input-49-1ccd80ee230a> in <module>()
----> 1 del fruits[30]
...
<ipython-input-50-c4712a765080> in <module>()
----> 1 fruits.remove('test')
...
In [23]: del com_dict['Doran\'s Blade']

In [24]: del com_dict['Doran\'s Ring']
...
```shell
# 6가지 색상의 영어 키, 한글 값을 갖는 딕셔너리(colors)를 생성한다. (red, green, blue, yellow, black, white)
In [1]: colors = {
   ...:     'red': '빨강',
   ...:     'green': '초록',
   ...:     'blue': '파랑',
...
# colors를 셋(Set)으로 만들어 colors_set변수에 할당한다.
In [4]: colors_set = set(colors)

# colors_set에 purple이 존재하는지 확인한다.
In [5]: 'purple' in colors_set
Out[5]: False

# [2,4,3,7,6,8,4,6,5,3,2,5,6,7,3]에서 중복된 값을 없애고, 오름차순으로 정렬한 list를 반환한다.
